...
class _MyStatelessWidgetState extends State<MyStatelessWidget> {
              ...
              child: SingleChildScrollView(
                ...
                child: DataTable(
                  ...
                  columns: <DataColumn>[
                    ...
                    _renderDataColomn("Total Accumulated Price $totalPrice", snapshot, context),
                      ...
                      .map(
                        (element) => DataRow(
                          cells: <DataCell>[
                            ...
                            _renderDataCell(Parsing.intFrom(element.terms.priceEach).toString()),
                            _renderDataCell(element.quantityOnHand.toString()),
                            _renderDataCell(_calculateEachTotal(element.quantityOnHand, element.terms.priceEach)),
                            _renderDataCell(_calculateTotalPv(element.quantityOnHand, element.terms.pvEach)),
                          ],
                        ),
                      )
                      .toList(),
  ...
  String _calculateTotalPrice(InventoryRecords inventoryRecords, String type ) {
    double total = 0.0;
      ...
      for (final item in inventoryRecords.items) {
        total += Parsing.intFrom(item.quantityOnHand)! * (type == "pv" ? item.terms.pvEach : item.terms.priceEach);
      }
  ...
  void _onSortColum(List<InventoryRecordItems> inventoryRecordItems, int columnIndex,
      bool ascending) {
    if (columnIndex == 0) {
      if (ascending) {
        inventoryRecordItems.sort((a, b) => a
            .item.id.unicity
            .compareTo(b.item.id.unicity));
      } else {
        inventoryRecordItems.sort((a, b) => b
            .item.id.unicity
            .compareTo(a.item.id.unicity));
      }
    }
      ...
      if (ascending) {
        inventoryRecordItems.sort((a, b) => a
            .terms.pvEach
            .compareTo(b.terms.pvEach));
      } else {
        inventoryRecordItems.sort((a, b) => b
            .terms.pvEach
            .compareTo(a.terms.pvEach));
      }
    }
      ...
      if (ascending) {
        inventoryRecordItems.sort((a, b) => a
            .terms.priceEach
            .compareTo(b.terms.priceEach));
      } else {
        inventoryRecordItems.sort((a, b) => b
            .terms.priceEach
            .compareTo(a.terms.priceEach));
      }
    }
  }
